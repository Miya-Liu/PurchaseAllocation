...
package ch.uzh.ifi.tg.serviceMarket.fedx.cache;

import java.util.ArrayList;
import java.util.List;

import ch.uzh.ifi.tg.serviceMarket.fedx.algebra.StatementSource;
import ch.uzh.ifi.tg.serviceMarket.fedx.exception.OptimizationException;
import ch.uzh.ifi.tg.serviceMarket.fedx.structures.Endpoint;
import ch.uzh.ifi.tg.serviceMarket.fedx.structures.SubQuery;
...
import ch.uzh.ifi.tg.serviceMarket.fedx.algebra.StatementSource;
import ch.uzh.ifi.tg.serviceMarket.fedx.algebra.StatementSource.StatementSourceType;
import ch.uzh.ifi.tg.serviceMarket.fedx.cache.Cache.StatementSourceAssurance;
...
public class CacheUtils {
	 ...
	 */
	private static boolean checkEndpointForResults(Cache cache, Endpoint endpoint, Resource subj, URI pred, Value obj) throws OptimizationException {
		try {
	 ...
	 * @return
	 */
	public static boolean checkCacheUpdateCache(Cache cache, List<Endpoint> endpoints, Resource subj, URI pred, Value obj) {
		...
		for (Endpoint e : endpoints) {
			...
			if (a== Cache.StatementSourceAssurance.HAS_LOCAL_STATEMENTS || a== Cache.StatementSourceAssurance.HAS_REMOTE_STATEMENTS)
				return true;	
			if (a== Cache.StatementSourceAssurance.POSSIBLY_HAS_STATEMENTS && checkEndpointForResults(cache, e, subj, pred, obj))
				return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static List<StatementSource> checkCacheForStatementSourcesUpdateCache(Cache cache, List<Endpoint> endpoints, Resource subj, URI pred, Value obj) {
		...
		for (Endpoint e : endpoints) {
			...
			} else if (a== Cache.StatementSourceAssurance.HAS_REMOTE_STATEMENTS) {
				sources.add( new StatementSource(e.getId(), StatementSource.StatementSourceType.REMOTE));
			} else if (a== Cache.StatementSourceAssurance.POSSIBLY_HAS_STATEMENTS) {
