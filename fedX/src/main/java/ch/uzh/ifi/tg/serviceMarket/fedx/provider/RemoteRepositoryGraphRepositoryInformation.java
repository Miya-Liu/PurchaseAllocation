...
public class RemoteRepositoryGraphRepositoryInformation extends RepositoryInformation {
	...
	public RemoteRepositoryGraphRepositoryInformation(String repositoryServer, String repositoryName) {
		super("remote_" + repositoryName, "http://"+repositoryName, repositoryServer + "/" + repositoryName, Endpoint.EndpointType.RemoteRepository);
	...
	protected void initialize(Model graph, Resource repNode) {
		...
		Model repositoryServer = graph.filter(repNode, new URIImpl("http://fluidops.org/config#repositoryServer"), null);
		...
		// repositoryName
		Model repositoryName = graph.filter(repNode, new URIImpl("http://fluidops.org/config#repositoryName"), null);
