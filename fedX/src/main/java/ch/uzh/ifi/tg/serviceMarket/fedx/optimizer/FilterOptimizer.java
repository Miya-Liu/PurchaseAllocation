 ...
 *
 */
public class FilterOptimizer extends QueryModelVisitorBase<OptimizationException> implements FedXOptimizer {
	...
	protected class VarFinder extends QueryModelVisitorBase<OptimizationException> {
	...
	protected class FilterExprInsertVisitor extends QueryModelVisitorBase<OptimizationException> {
		...
		private boolean handleCompare(FilterTuple filterTuple, Compare cmp) {
			...
			if (isVarLeft && cmp.getRightArg() instanceof ValueConstant) {
				...
				if (value instanceof Resource) {
					...
					insertedValues.add(new BindingImpl(varName, value));
					return true;
				}
			}
