...
public class SPARQLGraphRepositoryInformation extends RepositoryInformation {
	...
	protected void initialize(Model graph, Resource repNode) {
		...
		// location		
		Model location = graph.filter(repNode, new URIImpl("http://fluidops.org/config#SPARQLEndpoint"), null);
		...
		if (hasAdditionalSettings(graph, repNode)) {
			...
			if (graph.contains(repNode, new URIImpl("http://fluidops.org/config#supportsASKQueries"), ValueFactoryImpl.getInstance().createLiteral("false")))
	...
	protected boolean hasAdditionalSettings(Model graph, Resource repNode) {
		return graph.contains(repNode, new URIImpl("http://fluidops.org/config#supportsASKQueries"), null);
	}
}
